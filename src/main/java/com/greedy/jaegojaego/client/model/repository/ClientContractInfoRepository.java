...
import com.greedy.jaegojaego.client.model.dto.ClientContractInfoDTO;
...
public interface ClientContractInfoRepository extends JpaRepository<ClientContractInfo, Integer> {
    ...
    "               WHERE A.CLIENT_NO = ?1" , nativeQuery = true)
    List<ClientContractInfo> findByClient(int clientNo);

    ClientContractInfo findClientContractInfoNoByClient(int clientNo);

    ClientContractInfo findClientContractInfoNoByClient_ClientNo(int clientNo);
}
